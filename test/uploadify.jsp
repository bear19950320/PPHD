<html lang="en">
	...
	<head>
			...
			<script type="text/javascript">
				...
				function uploadMaxFile() {
					...
					if(file) {
						...
						testxhr.open("POST", "/UploaderDemo/uploadifyTest_getUploadedSize.action");
						testxhr.send(fd);
				...
				function uploadFile(file, start) {
					...
					//发送文件和表单自定义参数  
					xhr.open("POST", "/UploaderDemo/uploadifyTest_doUpload.action");
					xhr.send(fd);
				}
				//取消上传  
		...
		<body>
			<form id="form1" enctype="multipart/form-data" method="post" action="uploadify.jsp">
				<div class="row">
					<label for="fileToUpload">选择文件</label> <input type="file" name="file" id="file" onchange="fileSelected();" />
				</div>
				<div id="fileName"></div>
				<div id="fileSize"></div>
				<div id="fileType"></div>
				<div class="row">
					<input type="button" onclick="uploadMaxFile()" value="上传" /> <input type="button" onclick="cancleUploadFile()" value="取消" />
				</div>
